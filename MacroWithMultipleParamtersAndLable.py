...
if macroDefinition[0] == "MACRO" and macroDefinition[-1] == "MEND":
    ...
    labelName, macroName, *argName = temp.split()
    temp = argName
...
for i in range(sc):
    if macroName in sourceCode[i]:
        ...
        labelValue, macroName, *argValue = temp.split()
        ...
        for j in range(2, mc - 1):
            if labelName in macroDefinition[j]:
                temp = macroDefinition[j]
                tempLabel, opCode, value = temp.split()
                tempLabel = label[x]
                tempValue = mapping.get(value + str(x))
                temp = tempLabel + ' ' + opCode + ' ' + str(tempValue)
...
for i in range(len(totalArgs)):
    ...
    print("Actual arguement during {} Macro call = {}".format(
        i+1, ''.join(label[i][0:-1])))
